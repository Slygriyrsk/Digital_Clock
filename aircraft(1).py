...
class NetworkSim:
    ...
    def should_drop(self, chunk_idx):
        if self._in_blackout():
            ...
            remaining = cfg.NET_BLACKOUT_END_S - (time.time() - self._session_t0)
            print(f"  [BLACKOUT] chunk {chunk_idx} dropped  "
                  f"(blackout ends in {remaining:.1f}s)")
            return True

        if self._burst_count > 0:
            self._burst_count -= 1
            self.stats["burst_drops"] += 1
            print(f"  [BURST-DROP] chunk {chunk_idx} dropped  "
                  f"({self._burst_count} more in burst)")
            return True

        if self._rng.random() < cfg.NET_DROP_PROB:
            if self._rng.random() < 0.3:
                self._burst_count = cfg.NET_BURST_SIZE - 1
                print(f"  [BURST-START] chunk {chunk_idx} — burst of "
                      f"{cfg.NET_BURST_SIZE} dropping")
            else:
    ...
    def simulated_latency_ms(self):
        return cfg.NET_BASE_LATENCY_MS + self._rng.uniform(
            -cfg.NET_JITTER_MS, cfg.NET_JITTER_MS)
...
def load_audio_file(path):
    ...
    if sampwidth == 2:
        ...
        samples = [s / 32767.0 for s in samples]
    elif sampwidth == 1:
        ...
        fmt     = f"<{len(raw)//4}i"
        samples = list(struct.unpack(fmt, raw))
        samples = [s / 2147483647.0 for s in samples]
    else:
    ...
    print(f"  Audio norm: {TARGET_RATE}Hz mono 16-bit  "
